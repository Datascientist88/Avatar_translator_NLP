...
def create_video(text, source_language, target_language, image_data):
    ...
    draw.rectangle(
        [(25, 25), (avatar_width - 25, avatar_height - 25)], outline="white", width=10
    )
    draw.multiline_text(
        (25, 50),
        translated_text,
    ...
    clip_duration = (
        len(translated_text) // 5 + 1
    )  # Estimate clip duration based on text length
    clip_width = 640
    clip_height = 480
    background_color = (255, 255, 255)
    clip = mp.ImageClip(
        np.zeros((clip_height, clip_width, 3), dtype=np.uint8) + background_color
    )
    avatar_clip = (
        mp.ImageClip(np.array(avatar_image))
    ...
    for t in range(int(clip_duration * fps)):
        ...
        for (x, y, w, h) in faces:
            ...
            draw = ImageDraw.Draw(avatar_image)
            draw.rectangle(
                [(mouth_x1, mouth_y), (mouth_x2, mouth_y + 0.1 * h)], fill="white"
            )
    ...
    clip.write_videofile(
        video_file, audio=mp.AudioClip(speech_data), fps=fps, codec="libx264"
    )
    video_file.seek(0)
...
def update_upload_text(filename):
    if filename is not None:
        return html.Div([html.P(f"Selected file: {filename}")])
    else:
        return html.Div(["Drag and drop or click to select an image file to upload."])


# Define callback for translating text and creating video
@app.callback(
    Output("video-output", "src"),
...
def translate_and_create_video(
    n_clicks, text, source_language, target_language, image_data
):
    if n_clicks > 0:
        if image_data is not None:
            ...
            background_color = (255, 255, 255)
            clip = mp.ImageClip(
                np.zeros((clip_height, clip_width, 3), dtype=np.uint8)
                + background_color
            )
            image_clip = (
            ...
            # Add mouth movement to video clip
            fps = (
                clip.fps if clip.fps is not None else 25
            )  # Set default fps to 25 if fps is None
            frames = []
            for t in range(int(clip_duration * fps)):
                frame = clip.get_frame(t / fps)
                gray_image = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
                ...
                for (x, y, w, h) in faces:
                    ...
                    draw.rectangle(
                        [(mouth_x1, mouth_y), (mouth_x2, mouth_y + 0.1 * h)],
                        fill="white",
                    )
                image_data = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
            ...
            video_uri = "data:application/json;base64," + base64.b64encode(
